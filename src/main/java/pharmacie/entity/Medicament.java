...
public class Medicament {
    ...
    /**
     * Nombre d'unités en stock Décrémenté quand on expédie une commande
    ...
    /**
     * Nombre d'unités "en commande" Un médicament est "en commande" si il est
     * dans une commande qui n'est pas encore expédiée Incrementé quand on
     * ajoute des unités de ce médicament à une ligne de commande Décrémenté
     * quand on expédie une commande contenant ce médicament
     */
    @ToString.Exclude
    @PositiveOrZero
    ...
    /**
     * Niveau de reapprovisionnement Si le stock devient inférieur ou égal à ce
     * niveau, on doit approvisionner de nouvelles unités de ce médicament
    ...
    @OneToMany(mappedBy = "medicament", cascade = {CascadeType.PERSIST, CascadeType.MERGE, CascadeType.REFRESH, CascadeType.REMOVE})
